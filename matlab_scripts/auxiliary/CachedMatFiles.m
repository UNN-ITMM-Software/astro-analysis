classdef CachedMatFiles < dynamicprops
    ...
    methods(Access = private, Hidden = true)
        ...
        function fileVarName = getFileVarName(obj, varName)
            fileVarName = fullfile(obj.Properties.SourceDir, [varName, '.mat']);
        end
        ...
        function output = loadEntireVariable(obj, varName)
            if is_cached(obj, varName)
                output = obj.Cache.(varName);
            else
                varsFromFile = load(getFileVarName(obj, varName), varName, '-mat');
        ...
        function inefficientPartialSave(obj, indexingStruct, varName, value)
            ...
            save(getFileVarName(obj, varName), '-fromStruct', varsFromFile, '-append');
        ...
        function saveEntireVariable(obj, varName, value)
            ...
            save(getFileVarName(obj, varName), '-fromStruct', varsFromFile, '-v7.3');
        ...
        function varargout = genericWho(obj, fcnHan, fcnName, varargin)
            ...
            elseif length(varargin) == 1
                if is_cached(obj, varargin{1})
                    data = obj.Cache.(varargin{1});
                    [varargout{1:nargout}] = fcnHan('data');
                else
    ...
    methods(Access = public, Hidden = true)
        ...
        function obj = CachedMatFiles(Source, varargin)
            
            narginchk(1, inf);
            [SourceFile, SourceDir] = resolveSource(Source);
            obj.Properties = Properties(SourceFile, SourceDir);
            obj.IsCacheable = struct();
                    ...
                    switch length(indexingStruct)
                        ...
                        case 2
                            ...
                            if obj.Properties.SupportsPartialAccess
                                if is_cached(obj, varName)
                                    [varargout{1:nargout}] = subsref(obj.Cache.(varName), indexingStruct(2:end));
                                else
                                    [varargout{1:nargout}] = ...
                                        matlab.internal.language.partialLoad(getFileVarName(obj, varName), varSubset, '-mat');
                                end
                            else
                ...
                if strcmp(varName, 'Properties')
                    ...
                    switch length(indexingStruct)
                        ...
                        case 2
                            ...
                            varSubset = createMetaDescriptionOfIndexing(varName, varInfo, indexingStruct);
                            if obj.Properties.SupportsPartialAccess
                                recache(obj, varName, value, indexingStruct);
                                if ~is_cached(obj, varName)
                                    try
                                        matlab.internal.language.partialSave(getFileVarName(obj, varName), value, varSubset);
                                    catch
                                        warning('Efficient partial saving with indexing was unsuccessful. Try inefficient way...');
        ...
        function reload_cache(obj, varName)
            if ~is_created(obj, varName)
                obj = ...
                    subsasgn(obj, struct('subs', varName, 'type', '.'), []);
            end
            obj.IsCacheable.(varName) = false;
            obj.Cache.(varName) = ...
                subsref(obj, struct('subs', varName, 'type', '.'));
            obj.IsCacheable.(varName) = true;
        end
        
        function cache(obj, varName)
            if ~is_cached(obj, varName)
                reload_cache(obj, varName);
            end
        end
        
        function recache(obj, varName, value, indexingStruct)
            if is_cached(obj, varName)
                if nargin == 4
                    obj.Cache.(varName) = ...
                        subsasgn(obj.Cache.(varName), ...
                        indexingStruct(2:end), value);
                elseif nargin == 3
                    obj.Cache.(varName) = value;
                elseif nargin == 2
                    reload_cache(obj, varName);
                end
            end
        ...
        function uncache(obj, varName)
            if is_cached(obj, varName)
                obj.IsCacheable.(varName) = false;
                value = obj.Cache.(varName);
                obj.Cache = rmfield(obj.Cache, varName);
                indexingStruct = struct('subs', varName, 'type', '.');
                [~] = subsasgn(obj, indexingStruct, value);
            end
        end
        
        function ok = is_created(obj, varName)
            ok = isprop(obj, varName) && ...
                exist(getFileVarName(obj, varName), 'file') == 2;
        end
        
        function ok = is_cached(obj, varName)
            ok = isfield(obj.IsCacheable, varName) && ...
                obj.IsCacheable.(varName) == true;
        end
        
        function varargout = horzcat(~, varargin) %#ok<MANU,STOUT> Knowingly not using input or output.
            error(message('MATLAB:MatFile:CannotConcatenate'));
        end
        function varargout = vertcat(~, varargin) %#ok<MANU,STOUT> Knowingly not using input or output.
            error(message('MATLAB:MatFile:CannotConcatenate'));
        end
        function varargout = cat(~, varargin) %#ok<MANU,STOUT> Knowingly not using input or output.
            error(message('MATLAB:MatFile:CannotConcatenate'));
        end
...
function [sourceName, sourceDir] = resolveSource(sourceName)
    try
    ...
    [sourceDir, ~, ext] = fileparts(sourceName);
