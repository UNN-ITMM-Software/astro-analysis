function [fig] = plot_configurations(calculus, properties)
    if ~isfield(properties, 'info_log') || properties.info_log
        add_info_log('Painting configuration...');
    end
    ...
    hold off;
    if length(id_algo) == 2
        params.name = [variable_info.name, ' (ITMM) and (Yu Wei)'];
    end
    ...
    %%
    if ~isfield(properties, 'info_log') || properties.info_log
        add_info_log('Configuration painted.');
    end
end