...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    {        
        ...
        else if (mxIsUint16(input_video))
        {
            img_type = CV_16UC1;
        }
        else if (mxIsSingle(input_video))
        {
            img_type = CV_32F;
        }
        else
        ...
        uchar left_bound = LEFT_BOUND, right_bound = RIGHT_BOUND;
        if (mxIsStruct(prhs[nrhs - 1]))
        {
            const mxArray * param = prhs[nrhs - 1];
            int cnt = mxGetNumberOfFields(param);
            mexPrintf("Number of fields in the configuration structure = %d\n", cnt);
            for (int i = 0; i < cnt; i++)
            {
                std::string name_field = mxGetFieldNameByNumber(param, i);
                mxArray * cur = mxGetFieldByNumber(param, 0, i);
                if (name_field == "left_bound")
                {
                    if (mxIsUint8(cur))
                    {
                        left_bound = *(uchar *)mxGetData(cur);
                        mexPrintf("left_bound = %d\n", left_bound);
                    }
                }
                if (name_field == "right_bound")
                {
                    if (mxIsUint8(cur))
                    {
                        right_bound = *(uchar *)mxGetData(cur);
                        mexPrintf("right_bound = %d\n", right_bound);
                    }
                }
            }
        }
